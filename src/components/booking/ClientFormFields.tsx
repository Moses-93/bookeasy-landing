 ...
 * @returns Fragment with a main fieldset and an optional standalone comment field.
 */
  ...
  return (
    <>
      ...
      >
        ...
        {/* Core Field: Phone */}
        <div
          className={`flex flex-col px-4 py-2.5 ${customFields.length > 0 ? "border-b border-slate-100" : ""
        ...
        >
          ...
          <div className="relative flex items-center w-full bg-transparent">
            ...
            <input
              ...
              maxLength={15}
              required
              className={`w-full border-none bg-transparent p-0 text-sm text-slate-900 outline-none placeholder-slate-400 focus:ring-0 transition-all duration-200 ease-out ${flag ? "pl-11" : "pl-4"
                }`}
              value={phone.replace(/^\+/, "")}
              onChange={(e) => {
      ...
      {/* Comment Field */}
      {commentField && (
        <div className={`bg-white rounded-2xl border-[0.5px] border-slate-200/50 flex flex-col px-4 py-3 focus-within:bg-slate-50/50 transition-colors transition-opacity ${disabled ? "opacity-75" : ""}`}>
          <label
            htmlFor="comment_input"
            className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-1.5 flex items-center gap-1.5 cursor-pointer"
          >
            Коментар {commentField.isRequired ? <span className="text-red-500">*</span> : null}
          </label>
          <textarea
            id="comment_input"
            name="comment"
            ...
            className="w-full border-none bg-transparent p-0 text-sm text-slate-900 outline-none placeholder-slate-400 focus:ring-0 resize-none disabled:cursor-not-allowed"
          />
        </div>
      )}
    </>
